...
const userOut = u => ({ id: String(u.id), username: u.username, role: u.role, isActive: u.isActive });
...
users.get('/', async (req, res) => {
  const list = await models.User.findAll({ order: [['createdAt', 'ASC']] });
...
users.post('/', async (req, res) => {
  ...
  const user = await models.User.create({ username, passwordHash: await bcrypt.hash(password, 10), role });
